...
function generateTarckingId() {
  const prefix = "PRCL";
  const date = new Date().toISOString().slice(0, 10).replace(/-/g, "");
  const rendom = crypto.randomBytes(3).toString("hex").toUpperCase();

  return `${prefix}-${date}-${rendom}`;
}
...
const verifyFVToken = async (req, res, next) => {
...
async function run() {
  try {
    ...
    app.patch("/payment-success", async (req, res) => {
      ...
      // console.log("session reteieve", session);
      ...
      const trackingId = generateTarckingId();
    ...
    app.patch("/riders/:id", verifyFVToken, async (req, res) => {
